...
export default function Cadastro() {
  ...
  const [propsModalDeErroOuSucesso, setProsModalDeErroOuSucesso] = useState<{ exibir: boolean, titulo: string, corpo: string }>({ exibir: false, titulo: "", corpo: "" });
  ...
  const fecharModalDeErroOuSucesso = () => {
    setProsModalDeErroOuSucesso({ ...propsModalDeErroOuSucesso, exibir: false }); // ... => spread operator
  }

  const exibirModalDeErroOuSucesso = (titulo: string, corpo: string) => {
    setProsModalDeErroOuSucesso({ exibir: true, titulo, corpo });
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!nomeBolo || !categorias || !preco) { // se não coloquei bolo ou não coloquei categoria ou nâo coloquei preco
